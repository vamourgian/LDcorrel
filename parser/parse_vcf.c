    ...
    *word = (char*)malloc(sizeof(char)*STRINGLENGTH);
    ...
    if(Wset == 1) {
	    if(Wmin < 1 || Wmin > lines) {
	    	fprintf(stderr,"\n WARNING: Wmin must be between 1 and totalSnips, setting Wmin to 1 (default)\n");
	    	Wmin = 1;
	    }

	    if(Wmax < 1 || Wmax > lines) {
	    	fprintf(stderr,"\n WARNING: Wmax must be between 1 and totalSnips, setting Wmax to total Snips (default)\n");
	    	Wmax = lines;
	    }
	    ...
	    if(posWmin < 1) {
	    	fprintf(stderr,"\n WARNING: posWmin must be between 1 and 2147483646 (max positive int), setting posWmin to 1 (default)\n");
	    	posWmin = 1;
	    }

	    if(posWmax < 1) {
	    	fprintf(stderr,"\n WARNING: posWmax must be between 1 and 2147483646 (max positive int), setting posWmax to 2147483646 (default)\n");
	    	posWmax = 2147483646;
	    }

	    if(posWmin > posWmax) {
	    	fprintf(stderr,"\n WARNING: posWmin must be <= posWmax, setting posWmin = posWmax\n");
	    	posWmin = posWmax;
	    }
	}
	else if(inListSet == 1) {
    ...
    if(toSingleOutput == 0) {
	    ...
	    if(Wset == 1)
	    	status = createPartIndexW(fpIn, outputPathName, headerLine1, headerLine2, line, &lineLength, word, &wordLength, lines,size, &snipSize, allignmentId, & maxcount,&counter, &posMin, & posMax,Wmin,Wmax);
	    else if(posWset == 1)
			status = createPartPosW(fpIn, outputPathName, headerLine1, headerLine2, line, &lineLength, word, &wordLength, lines,size, &snipSize, allignmentId, & maxcount,&counter, &posMin, & posMax,posWmin,posWmax);
	    else if(inListSet == 1) {
			status = createPartPosL(fpIn, outputPathName, headerLine1, headerLine2, line, &lineLength, word, &wordLength, lines,size, &snipSize, allignmentId, & maxcount,&counter, &posMin, & posMax, inList,listSize);
	    	printf("\n");
	    	...
	    	status = createPart(fpIn, outputPathName, headerLine1, headerLine2, line, &lineLength, word, &wordLength, lines,size, &snipSize, allignmentId, & maxcount,&counter, &posMin, & posMax);
        ...
        status = createSingleFileIndexW(fpIn, outputPathName, &counter, line, &lineLength, word, &wordLength, lines, Wmin, Wmax);
	else if(toSingleOutput==1 && posWset ==1)
        status = createSingleFilePosW(fpIn, outputPathName, &counter, line, &lineLength, word, &wordLength, lines, posWmin, posWmax);
	else if(toSingleOutput==1 && inListSet ==1) {
        status = createSingleFilePosL(fpIn, outputPathName, &counter, line, &lineLength, word, &wordLength, lines, inList, listSize);
		printf("\n");
		for(inListSet=0;inListSet<listSize;inListSet++)
			if((*inList)[inListSet] != 0)
        		fprintf(stderr,"\tWARNING: list pos %d not found\n",(*inList)[inListSet]);
